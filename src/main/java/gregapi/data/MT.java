...
public class MT {
	...
	protected static OreDictMaterial create       (int aID, String aNameOreDict) {if (aID >= 16000 || aID < 15000 && aID >= 10000) return null; OreDictMaterial rMaterial = OreDictMaterial.createMaterial(aID, aNameOreDict, aNameOreDict); ALL_MATERIALS_REGISTERED_HERE.add(rMaterial); if (aID > 0) rMaterial.setOriginalMod(MD.GAPI); return rMaterial.handle(ANY.WoodPlastic);}
	...
	y       , Photon                    =   y       = create(   1, "Photon"     ).setStatsElement(0,0,0,0,0).heat(0,0,0).setRGBa(255, 255, 255, 255).setTextures(SET_NONE).put(PARTICLE).tooltip("y").hide(),
	v       , Neutrino                  =   v       = create(   2, "Neutrino"   ).setStatsElement(0,0,0,0,0).heat(0,0,0).setRGBa(180, 180, 180,   0).setTextures(SET_NONE).put(PARTICLE).tooltip("v").hide(),
	n       , Neutron                   =   n       = create(   3, "Neutron"    ).setStatsElement(0,0,1,0,0).heat(0,0,0).setRGBa(128, 128, 128,   0).setTextures(SET_NONE).put(PARTICLE).tooltip("n").hide(),
	p       , Proton                    =   p       = create(   4, "Proton"     ).setStatsElement(1,0,0,0,0).heat(0,0,0).setRGBa(255,   0,   0,   0).setTextures(SET_NONE).put(PARTICLE, FUSION).tooltip("p").hide(),
	e       , Electron                  =   e       = create(   5, "Electron"   ).setStatsElement(0,1,0,0,0).heat(0,0,0).setRGBa(  0,   0, 255,   0).setTextures(SET_NONE).put(PARTICLE).tooltip("e").hide(),
    HB      , HiggsBoson                =   HB      = create(   6, "Higgs-Boson").setStatsElement(0,0,0,0,0).heat(0,0,0).setRGBa(255, 255, 255, 255).setTextures(SET_NONE).put(PARTICLE).tooltip("HB").hide();
	...
	Li_6    = alkali      (  31, "Lithium-6"      , "Li-6"  ,   3,   3,   453,  1560,  0.534     , SET_RAD     , 230, 225, 255     ,      FUSION                                       , TICKS_PER_SMELT*10, WASHING_MERCURY                                       ).aspects(TC.VITREUS, 1, TC.POTENTIA, 1, TC.RADIO, 1     ).put(MOLTEN),
	...
	Be_7    = alkaline    (  41, "Beryllium-7"    , "Be-7"  ,   4,   3,  1560,  2742,  1.85      , SET_RAD     , 110, 190, 110     ,      FUSION                                                                                                                   ).aspects(TC.METALLUM, 2, TC.LUCRUM, 1, TC.RADIO, 1      ).put(MOLTEN),
	...
	Ru      = platingroup ( 440, "Ruthenium"      , "Ru"    ,  44,  57,  2607,  4423, 12.37      , SET_SHINY   , 155, 155, 155     , UUM        , IRON_GROUP                                                                                                       ).aspects_met_rad(3, 0                                   ).put(MOLTEN),
	Rh      = platingroup ( 450, "Rhodium"        , "Rh"    ,  45,  58,  2237,  3968, 12.41      , SET_SHINY   , 144, 144, 144     , UUM        , COBALT_GROUP                                                                                                     ).aspects_met_rad(3, 0                                   ).put(MOLTEN),
	...
	Hf      = refractmetal( 720, "Hafnium"        , "Hf"    ,  72, 106,  2506,  4876, 13.31      , SET_METALLIC, 140, 140, 150, 255, UUM        , TITANIUM_GROUP                       , FLAMMABLE, EXPLOSIVE                                                      ).aspects_met_rad(3, 0                                   ).put(MOLTEN),
	Ta      = refractmetal( 730, "Tantalum"       , "Ta"    ,  73, 107,  3290,  5731, 16.654     , SET_METALLIC, 120, 120, 140, 255, UUM        , VANADIUM_GROUP                                                                                                   ).aspects(TC.METALLUM, 2, TC.VINCULUM, 1                 ).put(MOLTEN),
	...
	U_238   = actinide    ( 920, "Uranium"        , "U"     ,  92, 146,  1405,  4404, 18.95      , SET_RAD     ,  50, 240,  50                                                         , MELTING, MOLTEN, "Uranium238", "Uran"                                     ).aspects_met_rad(2, 1                                   ).qual(3, 6.0, 512, 3).put(MOLTEN), // No, I could not make a Variable named "U" here. That would collide with the Material Unit.
	U_235   = actinide    ( 921, "Uranium-235"    , "U-235" ,  92, 143,  1405,  4404, 18.95      , SET_RAD     ,  70, 250,  70                                                         , MELTING, MOLTEN, "UraniumEnriched"                                        ).aspects_met_rad(1, 2                                   ).qual(3, 6.0, 512, 3).put(MOLTEN),
	...
	Pu      = actinide    ( 940, "Plutonium"      , "Pu"    ,  94, 150,   912,  3501, 19.84      , SET_RAD     , 240,  50,  50                                                         , "Plutonium244"                                                            ).aspects_met_rad(2, 1                                   ).qual(3, 6.0, 512, 3).put(MOLTEN),
	Pu_240  = actinide    ( 942, "Plutonium-240"  , "Pu-240",  94, 146,   912,  3501, 19.84      , SET_RAD     , 235,  30,  30                                                                                                                                     ).aspects_met_rad(1, 3                                   ).qual(3, 6.0, 512, 3),
	Pu_241  = actinide    ( 943, "Plutonium-241"  , "Pu-241",  94, 147,   912,  3501, 19.84      , SET_RAD     , 245,  70,  70                                                                                                                                     ).aspects_met_rad(1, 3                                   ).qual(3, 6.0, 512, 3).put(MOLTEN),
	Pu_243  = actinide    ( 945, "Plutonium-243"  , "Pu-243",  94, 149,   912,  3501, 19.84      , SET_RAD     , 250,  70,  70                                                                                                                                     ).aspects_met_rad(1, 2                                   ).qual(3, 6.0, 512, 3).put(MOLTEN),
	Pu_238  = actinide    ( 946, "Plutonium-238"  , "Pu-238",  94, 144,   912,  3501, 19.84      , SET_RAD     , 250,  30,  30                                                                                                                                     ).aspects_met_rad(1, 3                                   ).qual(3, 6.0, 512, 3),
	Pu_239  = actinide    ( 947, "Plutonium-239"  , "Pu-239",  94, 145,   912,  3501, 19.84      , SET_RAD     , 235,  50,  50                                                                                                                                     ).aspects_met_rad(1, 3                                   ).qual(3, 6.0, 512, 3).put(MOLTEN),
	Am      = actinide    ( 950, "Americium"      , "Am"    ,  95, 150,  1449,  2880, 13.69      , SET_RAD     , 200, 200, 200                                                                                                                                     ).aspects_met_rad(2, 1                                   ).qual(3, 4.0, 256, 2).put(MOLTEN),
	Am_241  = actinide    ( 951, "Americium-241"  , "Am-241",  95, 146,  1449,  2880, 13.69      , SET_RAD     , 210, 210, 210                                                                                                                                     ).aspects_met_rad(1, 3                                   ).qual(3, 4.0, 256, 2).put(MOLTEN),
	...
	Tn      = element( 1250, "TritaniumElemental"    , "Tn"      , 125, 198, 1500, 2500, 25.0     , SET_DULL     ,  55, 155, 155, 255, G_DUST_ORES, UUM, METAL, SMITHABLE, MELTING, MOLTEN, EXTRUDER, "Unbipentium").aspects(TC.METALLUM, 2, TC.TUTAMEN, 2, TC.VITREUS, 1).setLocal("Elemental Tritanium").put(FUSION), Ubp = Tn,
	Ke      = element( 1260, "Trinium"               , "Ke"      , 126, 192, 2645, 4523,  1.06874 , SET_METALLIC , 234, 234, 234, 255, G_INGOT_MACHINE_ORES, UUM, METAL, SMITHABLE, MELTING, MOLTEN, EXTRUDER, "Unbihexium").aspects(TC.METALLUM, 3, TC.TUTAMEN, 1, TC.VITREUS, 1).qual(3, 12.0, 2560, 4).put(FUSION), Ubh = Ke,
	Dn      = element( 1450, "DuraniumElemental"     , "Dn"      , 145, 190,  1200, 2100, 20.0, SET_DULL,  75, 175, 175, 255, G_DUST_ORES, UUM, METAL, SMITHABLE, MELTING, MOLTEN, EXTRUDER, "Unquadpentium").aspects(TC.METALLUM, 2, TC.TUTAMEN, 1).setLocal("Elemental Duranium").put(FUSION), Uqp = Dn, // material refined from dolamide ore
	Vb      = element( 1520, "Vibranium"             , "Vb"      , 152, 266,  4852, 9415, 3.23978365, SET_EMERALD, 200, 128, 255, 100, G_GEM_ORES_TRANSPARENT, UUM, VALUABLE, GLOWING, "Unpentbium").aspects(TC.VITREUS, 10, TC.SENSUS, 10).setPriorityPrefix(1).qual(3, 1000.0, 512, 15).put(FUSION), Upb = Vb,
	Nq      = element( 1740, "Naquadah"              , "Nq"      , 174, 352, 1500, 3000, 21.0, SET_RAD       ,  50,  50,  50, 255, G_INGOT_MACHINE_ORES, UUM, METAL, SMITHABLE, MELTING, MOLTEN, EXTRUDER, "Unseptquadium").aspects(TC.METALLUM, 3, TC.RADIO, 1, TC.NEBRISUM, 1).setRGBaLiquid(  0, 255,   0, 255).qual(3, 6.0, 1280, 4).put(FUSION), Usq = Nq,
	Nq_528  = element( 1741, "Naquadah-Enriched"     , "Nq-528"  , 174, 354, 1500, 3000, 22.0, SET_RAD       ,  60,  60,  60, 255, G_INGOT_ORES, METAL, SMITHABLE, MELTING, MOLTEN, EXTRUDER, EXPLOSIVE).aspects(TC.METALLUM, 3, TC.RADIO, 2, TC.NEBRISUM, 2).setRGBaLiquid( 64, 255,  64, 255).setLocal("Enriched Naquadah").qual(3, 6.0, 1280, 4).put(FUSION),
	Nq_522  = element( 1742, "Naquadria"             , "Nq-522"  , 174, 348, 1500, 3000, 20.0, SET_RAD       ,  30,  30,  30, 255, G_INGOT_ORES, METAL, SMITHABLE, MELTING, MOLTEN, EXTRUDER, EXPLOSIVE).aspects(TC.METALLUM, 4, TC.RADIO, 3, TC.NEBRISUM, 3).setRGBaLiquid(128, 255, 128, 255).qual(3, 1.0, 512, 4).put(FUSION),
	An      = element( 1850, "Abyssalnite"           , "An"      , 185, 396, 1500, 3000, 15.0, SET_METALLIC  ,  90,  40, 170, 255, G_INGOT_MACHINE_ORES, UUM, METAL, SMITHABLE, MELTING, MOLTEN, EXTRUDER, "Unoctpentium"                                ).qual(3, 8.0, 1600, 3).aspects(TC.METALLUM, 3, TC.VACUOS  , 2).put(FUSION), Uop = An,
	Cor     = element( 1860, "Coralium"              , "Cor"     , 186, 400, 2000, 4000, 20.0, SET_RUBY      ,  20, 160, 110, 255, G_INGOT_MACHINE_ORES, UUM, METAL, SMITHABLE, MELTING, MOLTEN, EXTRUDER, G_GEM_ORES, "Unocthexium", "LiquifiedCoralium").qual(3,12.0, 3200, 3).aspects(TC.METALLUM, 3, TC.PERDITIO, 2).put(FUSION), Uoh = Cor,
	Dr      = element( 1870, "Dreadium"              , "Dr"      , 187, 405, 2500, 5000, 25.0, SET_METALLIC  , 170,   0,   0, 255, G_INGOT_MACHINE_ORES, UUM, METAL, SMITHABLE, MELTING, MOLTEN, EXTRUDER, "Unoctseptium"                                ).qual(3,16.0, 4800, 4).aspects(TC.METALLUM, 3, TC.TENEBRAE, 2).put(FUSION), Uos = Dr,
	Etx     = element( 1880, "Ethaxium"              , "Etx"     , 188, 410, 3000, 6000, 30.0, SET_METALLIC  , 160, 170, 150, 255, G_INGOT_MACHINE_ORES, UUM, METAL, SMITHABLE, MELTING, MOLTEN, EXTRUDER, "Unoctoctium"                                 ).qual(3,20.0, 6400, 4).aspects(TC.METALLUM, 3, TC.VITIUM  , 2).put(FUSION), Uoo = Etx,
	Atl     = element( 2210, "Atlarus"               , "Atl"     , 221, 575, 3276, 11524, 21.24625421, SET_METALLIC, 204, 179,   0, 255, G_INGOT_MACHINE_ORES, UUM, SMITHABLE, MELTING, MOLTEN, EXTRUDER, MAGICAL, "Bibiunium").qual(3,  4.0, 4480, 4).aspects(TC.METALLUM, 2, TC.COGNITO, 1).put(FUSION), Bbu = Atl, Atlarus = Atl,
	Ad      = element( 2220, "Adamantium"            , "Ad"      , 222, 580, 5225, 14528, 13.35624762, SET_SHINY   , 255, 255, 255, 255, G_INGOT_MACHINE_ORES, UUM, SMITHABLE, MELTING, MOLTEN, EXTRUDER, MAGICAL, MAGNETIC_PASSIVE, WITHER_PROOF, ENDER_DRAGON_PROOF, "Adamant", "Bibibium").qual(3, 10.0, 5120, 5).aspects(TC.METALLUM, 10, TC.PRAECANTIO, 10).put(FUSION), Bbb = Ad,
	Mcg     = element( 2390, "Mac-Guffium"           , "Mcg"     , 239, 665, 200,  1000,  3.122, SET_SHINY, 200,  50, 150, 255, G_CONTAINERS, UUM, VALUABLE, GLOWING, LIGHTING, "Bitriennium").aspects(TC.ALIENIS, 8, TC.PERMUTATIO, 8, TC.SPIRITUS, 8, TC.AURAM, 8, TC.VITIUM, 8, TC.RADIO, 8, TC.MAGNETO, 8, TC.ELECTRUM, 8, TC.NEBRISUM, 8, TC.STRONTIO, 8).put(FUSION), Bte = Mcg,
	Gt      = element( 3720, "Gravitonium"           , "Gt"      , 372,1330, 112, 1275, 1768.866761, SET_SHINY,   0,  50,   0, 255, G_CONTAINERS, UUM, "Triseptbium").aspects(TC.TERRA, 10, TC.POTENTIA, 10).put(FUSION), Tsb = Gt,
	...
	Al2O3                   = oredustdcmp   ( 8008, "Alumina"               , SET_METALLIC          , 248, 248, 255, 255, MELTING, INGOTS)                                                                                                                          .uumMcfg( 0, Al             , 2*U, O                , 3*U)                                                                                                  .heat(2345, 3250),
...
//  HexoriumRainbow         = hexorium      ( 9242, 224, 224, 224, DYE_INDEX_White    ),
	...
	RedSteel                = alloymachine  ( 8632, "Red Steel"                                     , 140, 100, 100     , MOLTEN).qual(3,  7.0,  896, 2)                                                                                                            .uumAloy( 0, RoseGold       , 1*U, Brass            , 1*U, Steel            , 2*U, BlackSteel       , 4*U)                                                  .aspects(TC.METALLUM, 2, TC.ORDO, 1, TC.INSTRUMENTUM, 1),
	BlueSteel               = alloymachine  ( 8633, "Blue Steel"                                    , 100, 100, 140     , MOLTEN).qual(3,  7.5, 1024, 2)                                                                                                            .setAloy( 0, SterlingSilver , 1*U, BismuthBronze    , 1*U, Steel            , 2*U, BlackSteel       , 4*U)                                                  .aspects(TC.METALLUM, 2, TC.ORDO, 1, TC.INSTRUMENTUM, 1),
...
//          Al                      .addAlloyingRecipe(new OreDictConfigurationComponent( 1, OM.stack(OREMATS.Bauxite               , 2*U), OM.stack(K                  , 1*U)));
	...
	public static class OREMATS {
		...
		Chromite                = oredustelec( 9113, "Chromite"                  , SET_METALLIC  ,  35,  20,  15, 255, MORTAR, BLACKLISTED_SMELTER                                                    )                         .setMcfg( 0, Fe             , 1*U, Cr               , 2*U, O                , 4*U)                                                                        .qual(0),
