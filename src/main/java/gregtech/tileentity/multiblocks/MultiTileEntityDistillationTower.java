...
public class MultiTileEntityDistillationTower extends TileEntityBase10MultiBlockMachine {
	...
	static {
		...
		LH.add("gt.tooltip.multiblock.distillationtower.2", "3x3x12 of Distillation Tower Parts");
	...
	public void doOutputFluids() {
		for (IFluidTank tTank : mTanksOutput) {
			...
			if (tFluid != null && tFluid.amount > 0) {
				...
				if (FL.is(tFluid, "lubricant")) {
					tDelegator = WD.te(worldObj, getOffsetXN(mFacing, 3), yCoord+11, getOffsetZN(mFacing, 3), mFacing, F);
				} else if (FL.is(tFluid, "methane")) {
